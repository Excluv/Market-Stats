...
class DbAdapter:
    ...
    def _insert_product(self, table: str, products: pd.Series) -> None:
        ...
        products["assetclass_id"] = self.cursor.execute(
    ...
    def _insert_price_record(self, table: str, price_records: pd.DataFrame) -> None:
        ...
        # Find and insert the corresponding foreign key
        price_records["product_id"] = self.cursor.execute(
    ...
    def _remove_duplicates(self) -> None:
        ...
        Returns:
            None
                Directly affects the original dataset
        """
        self.data = self.data.drop_duplicates(subset=["date", "open", "high", "low", "close"])
...
class DataReader:
    ...
    def fetch_price_records(self, start_date: str, end_date: str, by="", arg="") -> list:
        ...
        self.cursor.execute(f"""
            SELECT
                assetclass.name, product.symbol, record.date,
...
class DbController:
    ...
    def _up_to_date(self, FILE_DIR: str, asset_class: str, end_date: datetime) -> bool or None:
        ...
        if asset_class != "Cryptocurrency":
            ...
            if weekday in timedelta_dict:
                yesterday = yesterday - timedelta_dict.get(weekday)

        if last_date_csv == end_date:
            return True
    ...
    def fetch_records(self, record_type: str, *args) -> list:
        ...
        return method_dict.get(record_type)(*arguments)
    ...
    def update_db(self, SOURCE: str, BASE_DIR: str) -> None:
        ...
        if SOURCE == "yfinance":
            ...
            for assetclass, symbol, alias, record_date in date_records:
                # Set the start date to be the day after the latest date available in DB
                start_date = pd.to_datetime(record_date, yearfirst=True).date() + pd.to_timedelta(1, unit="D")
                end_date = pd.Timestamp.today().date()

                # Download new a CSV file and append new data to the existing file
                DIR = os.path.join(os.path.abspath(BASE_DIR), assetclass)
                self.download_csv(DIR, assetclass, symbol, alias, None, start_date, end_date)
    ...
    def download_csv(self, BASE_DIR: str, asset_class: str,
        ...
        try:
            # Passing start and end dates as datetime objects will cause yfinance to
            # download data of different dates, convert to strings for consistency
            start_date = start_date.strftime("%Y-%m-%d")
            end_date = end_date.strftime("%Y-%m-%d")

            historical_data = yf.Ticker(alias).history(period=period, start=start_date, end=end_date)
            historical_data = pd.DataFrame(historical_data).to_csv(FILE_DIR, header=insert_header, mode="a")
        except IndexError: # When yfinance returns an empty Data Frame
            print(f"{symbol}: No new data records")
