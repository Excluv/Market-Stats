...
export default function Inflation(props) {
    ...
    if (chartData) {
      const chartComps = chartData.map((chartObject, index) => {
        return (
          <Column 
            ...
            config={index === 0 ? BigChartConfig : MediumChartConfig}
            childComponent={
              <div 
      ...
      setRowComps([
        ...
        <Row 
          key={`row-${2}`} 
          config={RowConfig} 
          childComponent={[chartComps[1], tableComps[1], chartComps[2], tableComps[2]]} 
        />, 
      ]);
    }
  }, [chartData]);
